...
void main(void) {
  ...
  if(n==vec3(0.0))
  {     // fragments outside mesh or that we do not want affected by RS
...
// Q? this assumes a directional light?
  ...
  if(display==0)
  {
    ...
    // blend in  manipulators
    float manipDepth = texture(depthMap, texCoord).r;
    if (manipDepth < gl_FragDepth)
    {  
       vec4 texel = texture(manipTTexture, texCoord.st);
       fcolour = vec4(mix(fcolour.rgb, texel.rgb, 0.4), fcolour.a);
    }
  }
  else if(display==1)
  {
